#include "circular_buffer_start.h"
...
ssize_t CBuffRead(c_buff_t *c_buff, void *buffer, size_t number_of_bytes)
{
	...
	/* number_of_bytes is what requested write_to_read is what i can read */
	number_of_bytes_to_read = MIN(number_of_bytes, c_buff -> write_to_read);

	c_buff -> write_to_read -= number_of_bytes_to_read; /* update how much i can read */
	...
	else
	{
		...
		c_buff -> read = c_buff -> circular_buffer_start + number_of_bytes_to_read;
	}
